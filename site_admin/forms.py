...
from core.widgets import MultiSearchWidget, DisplayOnlyWidget, \
    CheckboxSelectMultipleColumns
...
class ModifiedFormBase(Form):
    ...
    def as_table(self, field_name=None):
        """
        Just like Form.as_table(), except can render a single field (row)
        at a time.
        """
...
def autostrip(cls):
    """
    Convert a form so that each CharField's value is automatically
    stripped.
    """
    fields = [(key, value)
...
class CreateTagForm(Form):
    ...
    sectors = ModelMultipleChoiceField(
        queryset=Node.objects.getSectors(),
        label='Sectors',
        widget=CheckboxSelectMultipleColumns(columns=3),
        required=False)
    definition = CharField(
        widget=Textarea(),
        label='Definitiion',
        required=False)
    filters = ModelMultipleChoiceField(
        queryset=Filter.objects.all(),
        widget=CheckboxSelectMultipleColumns(columns=2),
        required=False,
        label='Filters')
    related_tags = MultiSearchField(
        model=Node,
        search_url='/admin/ajax/search_tags',
        label='Related Topics',
        widget_label='Associate Related Topics',
        show_create_tag_link=True,
        widget=MultiSearchWidget(
            remove_link_flyover_text='Remove Topic from Topic',
            blur_text='Type a few characters to bring up matching topics')
    )

    def __init__(self, user, *args, **kwargs):
        super(CreateTagForm, self).__init__(*args, **kwargs)
        self.fields['societies'] = \
            ModelMultipleChoiceField(
                queryset=Society.objects.getForUser(user),
                label='Organization',
                widget=CheckboxSelectMultipleColumns(columns=3))
    ...
    def clean_name(self):
        ...
        if self.user_role in [Profile.ROLE_ADMIN]:
            # If the name has changed check to see if it already exists
            # in the DB.
            ...
            if len(Node.objects.getTagsByName(data)):
                msg = "A topic named \"%s\" already exists. " \
                      "As a %s you do not have permission to create " \
                      "a duplicate tag. Only Administrators can do so." % \
                      (data, self.user_role.replace('_', ' ').title())
                raise forms.ValidationError(msg)
        return data
...
class EditTagForm(Form):
    ...
    societies = MultiSearchField(
        model=Society,
        search_url='/admin/ajax/search_societies',
        label='Organization',
        widget_label='Associate organizations',
        widget=MultiSearchWidget(
            remove_link_flyover_text='Remove Organization from Topic',
            blur_text='Type a few characters to bring up matching '
                      'organizations')
    )
    filters = ModelMultipleChoiceField(
        queryset=Filter.objects.all(),
    ...
    related_tags = MultiSearchField(
        ...
        search_url='/admin/ajax/search_tags',
        label='Related Topics',
        widget_label='Associate Related Topics with this Topic',
        show_create_tag_link=True,
        widget=MultiSearchWidget(
            remove_link_flyover_text='Remove Tag from Tag',
            blur_text='Type a few characters to bring up matching topics')
    )
...
class LoginForm(Form):
    ...
    password = CharField(widget=PasswordInput(), max_length=1000,
                         label='Password:')
...
class ForgotPasswordForm(Form):
    username = CharField(max_length=30, label='Please enter your username:',
                         required=False)
    email = CharField(label='Or the email associated with the account:',
                      required=False)
...
class CreateResourceForm(Form):
    ...
    nodes = MultiSearchField(label='Tags', model=Node,
                             search_url='/admin/ajax/search_tags')
    societies = MultiSearchField(model=Society,
                                 search_url='/admin/ajax/search_societies')
    priority_to_tag = BooleanField(required=False)
    completed = BooleanField(required=False)
    standard_status = ChoiceField(
        choices=_make_choices_with_blank(Resource.STANDARD_STATUSES),
        required=False)
...
class EditResourceForm(Form):
    ...
    resource_type = ModelChoiceField(
        queryset=ResourceType.objects.all(),
        widget=DisplayOnlyWidget(field_type=type(ModelChoiceField),
                                 use_capwords=True),
        label='Resource Type')
    ...
    nodes = MultiSearchField(
        label='Tags', model=Node, search_url='/admin/ajax/search_tags',
        widget_label='Associate Topics with this Resource',
        show_create_tag_link=True,
        widget=MultiSearchWidget(
            remove_link_flyover_text='Remove Topic from Resource',
            blur_text='Type a few characters to bring up matching topics'))
    societies = MultiSearchField(
        model=Society, search_url='/admin/ajax/search_societies',
        label='Organization',
        widget=MultiSearchWidget(
            remove_link_flyover_text='Remove Organization from Resource'))
    priority_to_tag = BooleanField(required=False)
    completed = BooleanField(required=False)
    standard_status = ChoiceField(
        choices=_make_choices(Resource.STANDARD_STATUSES), required=False)
    keywords = CharField(max_length=1000, required=False)
    year = IntegerField(required=False)
    date = DateField(required=False)
...
class SocietyForm(ModifiedFormBase):
    ...
    resources = MultiSearchField(model=Resource,
                                 search_url='/admin/ajax/search_resources')
...
class UserForm(Form):
    ...
    password1 = CharField(label='Enter a password:', max_length=100,
                          widget=PasswordInput(), required=False)
    password2 = CharField(label='Enter the password again:', max_length=100,
                          widget=PasswordInput(), required=False)
    role = ChoiceField(choices=list_to_choices(Profile.ROLES))
    societies = ModelMultipleChoiceField(queryset=Society.objects.all(),
                                         required=False)
...
class ManageSocietyForm(Form):
    ...
    # resources = MultiSearchField(model=Resource,
    #                              search_url='/admin/ajax/search_resources')
    tags = MultiSearchField(
        model=Node, search_url='/admin/ajax/search_tags',
        widget_label='Associate Topics with this Organization',
        show_create_tag_link=True,
        widget=MultiSearchWidget(attrs={
            'classMetadata': 'showSelectedOptions:false'
        }))


class MissingResourceForm(Form):
    society = ModelChoiceField(queryset=Society.objects.all(),
                               widget=HiddenInput())
    name = CharField(max_length=1000,
                     widget=DisplayOnlyWidget(field_type=CharField))
    email = CharField(max_length=1000,
                      widget=DisplayOnlyWidget(field_type=CharField))
    resource_type = ModelChoiceField(queryset=ResourceType.objects.all())
    description = CharField(label='Description of the missing resource',
                            widget=Textarea, max_length=5000)
...
class EditResourcesForm(Form):
    assign_tags = MultiSearchField(
        model=Node,
        search_url='/admin/ajax/search_tags',
        label='Assign Topics',
        widget_label='Assign Topics',
        show_create_tag_link=True,
        widget=MultiSearchWidget(
            remove_link_flyover_text='Remove Tag',
            blur_text='Type a few characters to bring up matching topics')
    )
    priority = ChoiceField(choices=TRISTATE_CHOICES_RESOURCES,
                           widget=RadioSelect())
    completed = ChoiceField(choices=TRISTATE_CHOICES_RESOURCES,
                            widget=RadioSelect())


class EditTagsForm(Form):
    emerging_technologies_filter = ChoiceField(
        choices=TRISTATE_CHOICES_TAGS, widget=RadioSelect(),
        label='Emerging Technologies')
    foundation_technologies_filter = ChoiceField(
        choices=TRISTATE_CHOICES_TAGS,
        widget=RadioSelect(), label='Foundation Technologies')
    hot_topics_filter = ChoiceField(
        choices=TRISTATE_CHOICES_TAGS, widget=RadioSelect(),
        label='Hot Topics')
    market_areas_filter = ChoiceField(
        choices=TRISTATE_CHOICES_TAGS, widget=RadioSelect(),
        label='Market Areas')
...
class EditClusterForm2(ModelForm):
    topics = MultiSearchField(
        model=Node, search_url='/admin/ajax/search_tags',
        widget=MultiSearchWidget(
            remove_link_flyover_text='Remove Topic from this Topic Area',
            blur_text='Type a few characters to bring up matching topics'),
        help_text="As you associate Topics with the Topic Area, "
                  "based upon your choices, the system will automatically "
                  "associate the Topic Area with relevant IEEE Organizations. "
                  "The system does so by searching for any Organizations that "
                  "are already associated with the Topic. "
                  "The Topic Area is then automatically associated with any "
                  "Organization already associated with one or more of the "
                  "Topic Area's Topics.")
    societies = ModelMultipleChoiceField(
        queryset=Society.objects.all(), label='Organization',
        widget=CheckboxSelectMultipleColumns(columns=3), required=False)
...
class EditClusterForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user_role == Profile.ROLE_SOCIETY_MANAGER:
            # Name is read-only
            self.fields['name'] = \
                CharField(widget=DisplayOnlyWidget(field_type=CharField))

            # Organizations are hidden
            self.fields['societies'].widget =  \
                self.fields['societies'].hidden_widget()
        ...
        elif user_role == Profile.ROLE_ADMIN and society is not None:
            # Organizations are hidden
            self.fields['societies'].widget = \
                self.fields['societies'].hidden_widget()
