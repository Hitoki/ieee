...
def checkout_site(): 
    ...
    # Apply collectstatic.
    run('cd "%(site_home)s/python" && source bin/activate && cd "%(site_home)s/python/releases/current/project/" && export PYTHONPATH=..:../../../lib/python2.7/site-packages/ && python "%(site_code)s/manage.py" collectstatic --noinput' % env)
...
def build_tagcloud_caches():
    _expand_site_paths(env)
    run('cd "%(site_home)s/python" && source bin/activate && export PYTHONPATH=%(site_code)s/..:%(site_code)s/:%(site_code)s/ieeetags/:$PYTHONPATH && export DJANGO_SETTINGS_MODULE=ieeetags.settings &&  python %(site_code)s/deploy/create_caches.py' % env)
