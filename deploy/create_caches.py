...
def main():
    ...
    for sector in sectors:
        print 'Creating cache for Sector %s sorted by %s' % \
              (sector.name, 'sort')
        insert_cache(sector, None, 'sector')

    societies = Society.objects.all()
    for society in societies:
        print 'Creating cache for Organization/Society %s sorted by %s' % \
              (society.name, 'sort')
...
def insert_cache(sector, society, page):
    ...
    sorts = ('connectedness', 'alphabetical', 'frequency', 'num_sectors',
             'num_related_tags', 'num_societies')
    for sort in sorts:
        cache_params['sort'] = sort
        content, node_count_content = _render_textui_nodes(
            sort, None, sector and sector.id or None, sector,
            society and society.id or None, society, None, None, True, True,
            False, page)
