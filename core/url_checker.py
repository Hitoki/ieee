...
from webapp.models.logs import UrlCheckerLog
...
class ArrayThreadSafe(object):
    ...
    def block_and_append(self, value):
        """
        Tries to add a value to the list.
        If the value exists already, block until it\'s removed.
        """
        self.lock.acquire()
        ...
        while True:
            ...
            if not result:
                ...
                # print 'ArrayThreadSafe.block_and_append(): adding "%s" to ' \
                #       'the array' % value
                self.array.append(value)
                break
            else:
...
def check_url(url, timeout=None):
    ...
    @return: A tuple (url_status, url_error), where url_status is 'good' or
    'bad' or '', and url_error is the error message.
    """
...
def check_url_thread(resource_queue, resources_to_save_queue, checking_hosts):
    ...
    while True:
        ...
        # print '  %s:   adding %s to checking_hosts' % (thread.getName(),
        #                                                hostname)
            ...
            if error_type == 'socket error':
                ...
                if type(e.args[1]) is socket.timeout:
                    ...
                    if error_num == 11001:
                        ...
                        resource.url_error = 'Error #%s: %s' % (error_num,
                                                                error_msg)
            else:
                # Unknown exception
        ...
        # print '  %s:   Removing %s from checking_hosts' % (thread.getName(),
        #                                                    hostname)
        ...
        # print '  %s: resource.url_status: %s' % (thread.getName(),
        #                                          resource.url_status)
        # print '  %s: resource.url_error: %s' % (thread.getName(),
        #                                         resource.url_error)
        resource.url_date_checked = datetime.datetime.now()

        # print '  %s: adding resource to resources_to_save_queue' % \
        #       (thread.getName())
...
def check_resources(resources, num_threads):
    ...
    @param num_threads The number of concurrent threads to use for checking
        URLs.
    ...
    for i in range(num_threads):
        thread = threading.Thread(
            name='Thread %s' % i,
            target=check_url_thread,
            args=[resource_queue, resources_to_save_queue, checking_hosts])
        thread.start()
    ...
    save_resources_thread = threading.Thread(target=save_resources_main,
                                             args=[resources_to_save_queue])
    ...
    while not resource_queue.empty():
        ...
        if log.date_ended is not None:
            ...
            if (time.clock() - last_time) > 0:
                speed = (last_num_resources - resource_queue.qsize()) / \
                        (time.clock() - last_time)
                log.status = '%s resources remaining (%s resources/sec).' % \
                             (resource_queue.qsize(), speed)
                # print '%s resources remaining (%s resources/sec).' % \
                #       (resource_queue.qsize(), speed)
                log.save()
                last_time = time.clock()
                last_num_resources = resource_queue.qsize()
    ...
    while len(threads):
        ...
        if len(threads):
            # Either wait for the first thread to end, or 2 seconds
