...
def _strip_quotes(str):
    ...
    if matches is None:
        raise Exception('Parameter %s is not in single or double quotes (ie. "%s" or \'%s\'' % (str, str, str))
...
def ifpermission(parser, token):
    ...
    Calls the Permission.objects.[permission_type]() function with the current user and the given object.  If true, renders the contents of this tag.  If false, renders nothing.
    """
    ...
    return IfPermissionNode(nodelist_true, nodelist_false, permission_type, object)
...
def ifnotpermission(parser, token):
    ...
    if token.contents == 'else':
        ...
        nodelist_false = template.NodeList()
    # NOTE: Since this is the NOT tag, nodelist_true and nodelist_false are switched...
    return IfPermissionNode(nodelist_false, nodelist_true, permission_type, object)
