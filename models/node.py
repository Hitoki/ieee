...
class NodeManager(models.Manager):
    ...
    # """
    # Returns a list of sectors whose names match the given list of
    # names.
    #     """
    ...
    #     """
    #     Returns the min/max amount of resources-per-tag for the given
    #     sector
    #     """
    ...
    #     """
    #     Returns the min/max amount of sectors-per-tag for the given
    #     sector.
    #     """
    #     #TODO: Should use tag.get_sectors() here instead?
    #     sector_counts = [tag.parents.count() for tag in
    #                      sector.child_nodes.all()]
    #
    ...
    #     """
    #     Returns the min/max amount of related_tags-per-tag for the given
    #     sector
    #     """
    ...
    def get_orphan_tags(self):
        ...
        tab_society_tags = [tag for tag in tab_society.tags.all() if
                            tag.societies.count() == 1]
    ...
    def get_clusters(self):
        ...
        """
        return self.filter(
            node_type=NodeType.objects.getFromName(NodeType.TAG_CLUSTER)).all()
...
class Node(models.Model):
    ...
    def parent_cluster_names(self):
        return ', '.join([cluster.name
                          for cluster in self.get_parent_clusters()])
            ...
            if add_child_info:
                ...
                for child_node in self.child_nodes.all():
                    for society in child_node.societies.all():
                        if not NodeSocieties.objects. \
                                filter(node=self, society=society).exists():
    ...
    def get_combined_sector_ranges(self, tags=None, show_empty_terms=False):
        ...
        """
        assert (self.node_type.name == NodeType.ROOT or NodeType.SECTOR
                or self.node_type.name == NodeType.TAG_CLUSTER), \
            'self (%s, %s, %s) must be a sector or cluster' % \
            (self.name, self.id, self.node_type.name)
        if tags is None:
            if self.node_type.name == NodeType.ROOT:
